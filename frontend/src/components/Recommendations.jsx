        ...
        <section className="recommendations">
            <div className="car-grid">
                {items.map((car) => {
                    ...
                    return (
                        <div key={carId} className="car-card" onClick={() => navigate(`/product/${carId}`)}>
                            <div className="car-image">
                                ...
                                />
                                <button
                                    className={`favorite-btn ${isFavorite(carId) ? 'active' : ''}`}
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        toggleFavorite(carId);
                                    }}
                                >
                                    <Heart size={18} fill={isFavorite(carId) ? "currentColor" : "none"} />
                                </button>
                            </div>
