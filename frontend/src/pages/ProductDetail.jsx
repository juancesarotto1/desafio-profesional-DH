...
const ProductDetail = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const [productRes, occupiedRes] = await Promise.all([
                    api.get(`/products/${id}`),
                    api.get(`/products/${id}/occupied-dates`)
                ]);

                setCar(productRes.data);

                // convertir YYYY-MM-DD strings a local Date objects
                const dates = occupiedRes.data.map(d => {
    ...
    const handleRating = async (stars) => {
        ...
        try {
            // opcional: POST valoracion a backend
            // esperar a que se envie la puntuacion api.post(`/products/${id}/rate`, { stars });
            setUserRating(stars);
    ...
    const handleContinueBooking = () => {
        if (!user) {
            ...
            console.log('Continuing to booking with:', { id: car.id, start, end });
            navigate(`/booking/${car.id}?start=${start}&end=${end}`);
        }
    };
                            ...
                            <div className="specs-grid">
                                <div className="spec-card">
                                    ...
                                    <div className="spec-info">
                                        <label>Consumo</label>
                                        <span>{finalSpecs.consumption}</span>
                                    </div>
                                </div>
                                <div className="spec-card">
                                    ...
                                    <div className="spec-info">
                                        ...
                                        <span>{finalSpecs.transmission}</span>
                                    </div>
                                </div>
                                <div className="spec-card">
                                    <div className="spec-icon"><Wind size={20} /></div>
                                ...
                                <div className="spec-card">
                                    ...
                                    <div className="spec-info">
                                        ...
                                        <span>{finalSpecs.drivetrain}</span>
                                    </div>
                                </div>
                                <div className="spec-card">
