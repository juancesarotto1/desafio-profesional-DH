...
const MyBookings = () => {
    ...
    return (
            ...
            <div className="bookings-list">
                {sortedBookings.length === 0 ? (
                    ...
                    sortedBookings.map(b => {
                        ...
                        const endDate = new Date(b.endDate + 'T23:59:59'); // End of that day
                        const isPast = endDate < new Date();

                        return (
                            <div key={b.id} className={`booking-item glass shadow ${isPast ? 'past-booking' : ''}`}>
                                <div className="item-img" style={{ filter: isPast ? 'grayscale(100%)' : 'none' }}>
                                ...
                                <div className="item-content">
                                    <div className="item-header">
                                        ...
                                        <div className="status-group">
                                            {isPast ? (
                                                <div className="status-badge finished">Finalizada</div>
                                            ) : (
                                                <>
                                                    <div className="status-badge confirmed">Confirmada</div>
                                    ...
                                    <div className="item-details" style={{ opacity: isPast ? 0.7 : 1 }}>
        ...
        .booking-item { display: flex; border-radius: 16px; overflow: hidden; border: 1px solid var(--border); transition: transform 0.2s; background: white; }
        .booking-item:hover { transform: scale(1.01); }
        .booking-item.past-booking { background: #f9f9f9; border-color: #eee; }
        ...
        .status-badge.confirmed { background: #e6ffed; color: #25d366; }
        .status-badge.finished { background: #eee; color: #888; border: 1px solid #ddd; }
        ...
        .item-details { display: flex; gap: 32px; margin-top: 16px; border-top: 1px dashed var(--border); padding-top: 16px; }
        .detail-box { display: flex; align-items: center; gap: 10px; color: var(--text-muted); font-size: 0.9rem; font-weight: 600; }
        .view-btn { 
            padding: 0 24px; 
            display: flex; 
            align-items: center; 
        ...
        .loc { font-size: 0.85rem; color: var(--text-muted); display: flex; align-items: center; gap: 4px; margin-top: 4px; font-weight: 600; }
        .item-header h3 { font-size: 1.4rem; font-weight: 800; }
      `}</style>
        </div>
    );
};
